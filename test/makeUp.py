...
# 注意limit
SOURCE_TABLE = "spark.origin_02"
SINK_TABLE = "spark.ttt02"
...
def getPoints(car_infoDF):
    ...
    res = {"map_longitude": map_longitudeL,
           ...
           "gps_time": gps_timeL}
    resDF = DataFrame(res)
    spark.createDataFrame().write.mode("overwrite").saveAsTable(SINK_TABLE)


if __name__ == '__main__':
    SOURCE_TABLE = spark.sql(f'''
            SELECT map_longitude, map_latitude , gps_time , car_number  from {SOURCE_TABLE}  ORDER BY gps_time limit 100
